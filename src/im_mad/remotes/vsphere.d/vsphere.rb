...
rc, data = do_action("/usr/lib/vmware-vcli/apps/host/hostinfo.pl --username #{@user} --password #{@pass} --server #{host}")
if rc == false
  exit info
end

data.split(/\n/).each{|line|
  mdata = line.match("^Physical Memory:\ *(.*) bytes")
  if mdata
    $total_memory = mdata[1].strip.to_i / 1024
    next
  end
  mdata = line.match("^Memory Usage:\ *(.*)MB")
  if mdata
    $used_memory = mdata[1].strip.to_i * 1024
    next
  end
  mdata = line.match("^Cpu Usage:\ *(.*)$")
  if mdata
    $used_cpu = mdata[1].strip.to_f / 1000
    next
  end
  mdata = line.match("^Cpu Speed:\ *(.*)$")
  if mdata
    $total_cpu = mdata[1].strip.to_f / 1000
    next
  end
...
add_info("USEDCPU",$used_cpu)
add_info("TOTALMEMORY",$total_memory)
add_info("FREEMEMORY",free_memory)
